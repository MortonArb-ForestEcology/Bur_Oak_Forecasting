...
# 0. Setup
# -------------------------------------------------
path.shiny <- "MortonArb_Buroak_Forecast/data_raw/meteorology/"
...
#-------------------------------#
#Pulling in daymet data to calculate PTT using daylength from daymet
...
lat.list <- daymetr::download_daymet(site = "Morton", lat = lat.in, lon = lon.in,
                                     start = 2021,
                                     end = 2021,
                                     internal = T)
...
length(lat.list)
names(lat.list) <- "Morton" # Giving the different layers of the list the site names they correspond to

#Lets look at the structure of what we are given
summary(lat.list)

lat.df <- as.data.frame(lat.list[[1]])
...
calc.indices <- function(dat){
  ...
  dat$PTTGDD5 <- (ifelse(dat$TMEAN>5, dat$TMEAN-5, 0) * (dat$DAYLEN/86400))
  dat$CDD0 <- ifelse(dat$TMEAN<0, 0-dat$TMEAN, 0)
  dat$CDD2 <- ifelse(dat$YDAY>172 & dat$TMEAN< -2, -2-dat$TMEAN, 0)
  dat$DaysNoRain <- NA
  dat$GTmean <- mean(dat[dat$YDAY <=90,"TMEAN"], na.rm = TRUE)
  
  dat[, c("GDD0.cum", "GDD5.cum", "CDD0.cum", "CDD2.cum", "PRCP.cum", "PTTGDD5.cum")] <- cumsum(dat[, c("GDD0", "GDD5", "CDD0", "CDD2", "PRCP", "PTTGDD5")])
...
met.ghcn$DAYLEN <- lat.df$dayl..s.[match(met.ghcn$YDAY, lat.df$yday)]
...
dat.gefs$DAYLEN <- lat.df$dayl..s.[match(dat.gefs$YDAY, lat.df$yday)]
...
dat.cfs$DAYLEN <- lat.df$dayl..s.[match(dat.cfs$YDAY, lat.df$yday)]
...
forecast.final$DAYLEN <- lat.df$dayl..s.[match(forecast.final$YDAY, lat.df$yday)] 
