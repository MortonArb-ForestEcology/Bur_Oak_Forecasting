...
weather_calc <- function(met.all, g_start=1, g_end=60){
  ...
  met.all$CDD5 <- ifelse(met.all$tmin..deg.c.<5, met.all$tmin..deg.c.-0, 0)
  ...
  for(YR in unique(met.all$year)){
    ...
    gdd5.cum=0; gdd0.cum=0; pttgdd.cum=0; cdd5.cum=0;
    d5.miss = 0; d0.miss=0; ptt.miss=0; c5.miss = 0;
    ncd = 0
    for(i in 1:nrow(dat.tmp)){
      ...
      if(is.na(dat.tmp$CDD5[i]) & c5.miss<=3){
        c5.miss <- c5.miss+1 # Let us miss up to 3 consecutive days
        cdd5.cum <- cdd5.cum+0
      } else {
        c5.miss = 0 # reset to 0
